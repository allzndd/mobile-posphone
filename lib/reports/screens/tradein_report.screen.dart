...
import '../services/tradein_report_service.dart';

class TradeInReportScreen extends StatefulWidget {
  final DateTime startDate;
  final DateTime endDate;

  const TradeInReportScreen({
    super.key,
  ...
  @override
  State<TradeInReportScreen> createState() => _TradeInReportScreenState();
}

class _TradeInReportScreenState extends State<TradeInReportScreen>
  ...
  Future<void> _loadData({bool isRefresh = false}) async {
    ...
    try {
      ...
      debugPrint('🔄 Loading trade-in report data...');
      debugPrint('📅 Date range: $startDate - $endDate');

      final results = await Future.wait([
        TradeInReportService.getTradeInSummary(
          startDate: startDate,
          endDate: endDate,
          storeId: _getStoreId(_selectedStore),
        ),
        TradeInReportService.getTradeInTransactions(
          startDate: startDate,
          endDate: endDate,
          page: _currentPage,
          perPage: _perPage,
        ...
        setState(() {
          ...
          if (results[1]['success'] == true) {
            ...
            if (newTransactions.isNotEmpty) {
              debugPrint('📊 First transaction structure: ${newTransactions[0]}');
            }
  ...
  Future<void> _loadStores() async {
    try {
      final result = await TradeInReportService.getStores();
  ...
  Future<void> _loadMoreData() async {
    ...
    try {
      ...
      final response = await TradeInReportService.getTradeInTransactions(
        startDate: startDate,
        endDate: endDate,
        page: _currentPage,
        perPage: _perPage,
  ...
  Widget _buildTopSummaryCards(
    ...
    final totalTradeIns = _summary['total_trade_ins'] ?? 0;
    final totalTradeInValue = _summary['total_trade_in_value'] ?? 0;
    final totalNewValue = _summary['total_new_value'] ?? 0;
    final averageTradeInValue = _summary['average_trade_in_value'] ?? 0;

    final cards = [
      {
        'label': 'Total Trade-Ins',
        'value': totalTradeIns.toString(),
        'icon': Icons.sync_alt_rounded,
        'color': const Color(0xFF9333EA), // Purple
      },
      {
        'label': 'Total Trade-In Value',
        'value': _formatCurrency(totalTradeInValue),
        'icon': Icons.attach_money_rounded,
        'color': const Color(0xFF22C55E), // Green
      },
      {
        'label': 'Total New Value',
        'value': _formatCurrency(totalNewValue),
        'icon': Icons.shopping_bag_rounded,
        'color': const Color(0xFF3B82F6), // Blue
      },
      {
        'label': 'Average Trade-In Value',
        'value': _formatCurrency(averageTradeInValue),
  ...
  Widget _buildTransactionGridCard(dynamic transaction, bool isMobile) {
    ...
    String oldProduct = '-';
    String newProduct = '-';
    dynamic transactionValue = 0;

    try {
      final pelanggan = transaction['pelanggan'];
      if (pelanggan != null && pelanggan is Map) {
        customerName = pelanggan['nama']?.toString() ?? '-';
      }

      final toko = transaction['toko'];
      if (toko != null && toko is Map) {
        storeName = toko['nama']?.toString() ?? '-';
      }

      final produkMasuk = transaction['produk_masuk'];
      if (produkMasuk != null && produkMasuk is Map) {
        oldProduct = produkMasuk['nama']?.toString() ?? '-';
      }

      final produkKeluar = transaction['produk_keluar'];
      if (produkKeluar != null && produkKeluar is Map) {
        newProduct = produkKeluar['nama']?.toString() ?? '-';
      }

      final transaksi = transaction['transaksi'];
      if (transaksi != null) {
        if (transaksi is List && transaksi.isNotEmpty) {
          transactionValue = transaksi[0]['total_harga'] ?? 0;
        } else if (transaksi is Map) {
          transactionValue = transaksi['total_harga'] ?? 0;
        }
      }
    } catch (e) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Amount
              Text(
                _formatCurrency(transactionValue),
                style: TextStyle(
                  fontWeight: FontWeight.bold,
          ...
          children: [
            ...
            Text(
              _searchController.text.isNotEmpty
                  ...
                  : 'No trade-in data available for this period',
              style: TextStyle(
                fontSize: isMobile ? 13 : 14,
                color: themeProvider.textSecondary,
              ),
  ...
  Future<void> _doExport() async {
    try {
      ...
      final result = await TradeInReportService.exportTradeInReport(
        startDate: startDate,
        endDate: endDate,
...
class _TransactionDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      ...
      debugPrint('Error parsing transaction detail data: $e');
    }
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Trade-In Card
                      _buildTradeInCard(themeProvider, isMobile),
                      ...
                      // Products Section
                      _buildProductsSection(themeProvider, isMobile, oldProduct, newProduct),
                      SizedBox(height: isMobile ? 12 : 16),

                      // Transaction Value Card
                      _buildTransactionValueCard(themeProvider, isMobile, transactionValue),
  ...
  Widget _buildTransactionValueCard(
    ThemeProvider themeProvider,
    bool isMobile,
