...
class _EditAdminScreenState extends State<EditAdminScreen> {
  ...
  void initState() {
    ...
    _selectedStoreId = widget.admin.storeId;
    _loadStores();
  }
  ...
  Future<void> _updateAdmin() async {
    ...
    try {
      final result = await UserManagementService.updateAdminUser(
        id: widget.admin.id,
        nama: _namaController.text.trim(),
        email: _emailController.text.trim(),
        password: _changePassword ? _passwordController.text : null,
        passwordConfirmation:
            _changePassword ? _passwordConfirmationController.text : null,
      ...
      if (mounted) {
        if (result['success'] == true) {
          await ValidationHandler.showSuccessDialog(
            context: context,
            title: 'Success',
            message: result['message'] ?? 'Admin account has been updated successfully!',
            onPressed: () {
          ...
          if (result['errors'] != null) {
            ...
            await ValidationHandler.showErrorDialog(
              context: context,
              title: 'Error',
              message: result['message'] ?? 'Failed to update admin. Please try again.',
            );
          }
        }
      }
      ...
      body: Form(
        ...
        child: CustomScrollView(
          slivers: [
            ...
            if (_changePassword)
              SliverToBoxAdapter(
                child: _buildPasswordSection(themeProvider, isMobile),
              ),
    ...
    return Container(
      ...
      child: _buildSectionCard(
        ...
        children: [
          ...
          SizedBox(height: isMobile ? 16 : 20),
          _buildChangePasswordToggle(themeProvider, isMobile),
        ],
        themeProvider: themeProvider,
        isMobile: isMobile,
      ),
  ...
  Widget _buildPasswordSection(ThemeProvider themeProvider, bool isMobile) {
    return Container(
      margin: EdgeInsets.fromLTRB(
        isMobile ? 16 : 24,
        0,
  ...
  Widget _buildChangePasswordToggle(ThemeProvider themeProvider, bool isMobile) {
    return Container(
      padding: EdgeInsets.all(isMobile ? 12 : 16),
      decoration: BoxDecoration(
        color: themeProvider.backgroundColor,
