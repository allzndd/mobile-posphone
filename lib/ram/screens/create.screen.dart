...
class RamCreateScreen extends StatefulWidget {
  const RamCreateScreen({super.key});

  @override
  State<RamCreateScreen> createState() => _RamCreateScreenState();
}

class _RamCreateScreenState extends State<RamCreateScreen> {
  ...
  Future<void> _saveRam() async {
    ...
    try {
      final response = await RamService.createRam(
      ...
      if (response['success'] == true) {
        await ValidationHandler.showSuccessDialog(
          context: context,
          title: 'Success',
          message: response['message'] ?? 'RAM created successfully',
          onPressed: () {
        ...
        if (response['errors'] != null) {
          ...
          await ValidationHandler.showErrorDialog(
            context: context,
            title: 'Error',
            message: response['message'] ?? 'Failed to create RAM',
          );
        }
      }
    } catch (e) {
      ...
      body: Form(
        ...
        child: CustomScrollView(
          slivers: [
            ...
            SliverToBoxAdapter(
              child: _buildRamInfoSection(themeProvider, isMobile),
            ),
            SliverToBoxAdapter(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: 'Enter RAM capacity (e.g., 4GB, 8GB)',
              ...
              errorText: _fieldErrors['kapasitas'],
            ),
            validator: (value) {
  ...
  Widget _buildSubmitButton(ThemeProvider themeProvider, bool isMobile) {
    return Container(
      ...
      child: ElevatedButton(
        onPressed: _isLoading ? null : _saveRam,
