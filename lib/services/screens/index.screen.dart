...
class _ServiceIndexScreenState extends State<ServiceIndexScreen>
  ...
  Future<void> _loadServices({bool isRefresh = false, int? page}) async {
    if (page != null) {
      _currentPage = page;
    } else if (isRefresh) {
      _currentPage = 1;
    ...
    try {
      ...
      if (response['success'] == true && mounted) {
        ...
        final int totalItems = response['pagination']?['total'] ?? serviceData.length;
  ...
  Widget _buildServicesListContainer(bool isDesktop, bool isTablet) {
    return Padding(
      ...
      child: Column(
        children: _services
            .map((service) => _buildModernServiceCard(service, isDesktop))
            .toList(),
      ),
    );
  }
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Flexible(
                  child: Text(
                    'Showing ${_services.isEmpty ? 0 : ((_currentPage - 1) * _perPage) + 1} - ${(_currentPage * _perPage) > _totalItems ? _totalItems : (_currentPage * _perPage)} of $_totalItems items',
                    style: TextStyle(
                      color: themeProvider.textSecondary,
                      fontSize: isDesktop ? 14 : 12,
                    ),
